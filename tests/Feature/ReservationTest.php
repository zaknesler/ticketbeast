...
class ReservationTest extends TestCase
{
    ...
    function completing_a_reservation_returns_an_order()
    {
        ...
        $order = $reservation->complete($paymentGateway, $paymentGateway->getValidTestToken(), 'fake_acct_1234');

        $this->assertEquals('john@example.com', $order->email);
        $this->assertEquals(3, $order->ticketQuantity());
        $this->assertEquals(3600, $order->amount);
        $this->assertEquals(3600, $paymentGateway->totalChargesFor('fake_acct_1234'));
    }
}
